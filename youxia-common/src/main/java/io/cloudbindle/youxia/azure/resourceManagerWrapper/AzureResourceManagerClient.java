...
public class AzureResourceManagerClient {
    ...
    private String getEndpoint(String endpoint, int attempts) throws Exception {
        for (int i = 0;; i++) {
            HierarchicalINIConfiguration youxiaConfig = ConfigTools.getYouxiaConfig();
            AuthenticationResult accessTokenFromUserCredentials = getAccessTokenFromUserCredentials();
            String accessToken = accessTokenFromUserCredentials.getAccessToken();
    ...
    private String patchEndpoint(String endpoint, String payload, int attempts) throws Exception {
        for (int i = 0;; i++) {
            HierarchicalINIConfiguration youxiaConfig = ConfigTools.getYouxiaConfig();
            AuthenticationResult accessTokenFromUserCredentials = getAccessTokenFromUserCredentials();
            String accessToken = accessTokenFromUserCredentials.getAccessToken();
    ...
    public ResourceGroupList getResourceGroupList() throws Exception {
        String endpoint = getEndpoint("resourcegroups", DEFAULT_ATTEMPTS);
        ResourceGroupList fromJson = gson.fromJson(endpoint, ResourceGroupList.class);
        return fromJson;
    }

    public ResourceGroup getResourceGroup(String name, int attempts) throws Exception {
        String endpoint = getEndpoint("resourcegroups/" + name, attempts);
        ResourceGroup fromJson = gson.fromJson(endpoint, ResourceGroup.class);
        return fromJson;
    }

    public ResourceGroup patchResourceGroup(String name, Map<String, String> newTags, int attempts) throws Exception {
        TagPatch patch = new TagPatch(newTags);
        String endpoint = patchEndpoint("resourcegroups/" + name, gson.toJson(patch), attempts);
        ResourceGroup fromJson = gson.fromJson(endpoint, ResourceGroup.class);
        return fromJson;
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            ResourceGroup fromJson = client.getResourceGroup(targetName, 1);
        ...
        try {
            ...
            ResourceGroup initialGroup = client.getResourceGroup(targetName, 1);
            Map<String, String> tags = initialGroup.getTags();
            tags.put(String.valueOf(System.currentTimeMillis()), "funky_new_tag");
            ResourceGroup updateResourceGroup = client.patchResourceGroup(targetName, tags, DEFAULT_ATTEMPTS);
